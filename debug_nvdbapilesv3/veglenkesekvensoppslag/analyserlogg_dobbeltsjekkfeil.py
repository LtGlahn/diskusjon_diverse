...
if __name__ == "__main__":
    ...
    datafil = 'data_fylke15viatech/fmekall_veglenke.log'
    print( 'Åpner FME loggfil oppsummering', datafil )
    f = open( datafil)
    for linje in f: 
    ...
    differanse = []
    feilsituasjoner = []
    normalstatus = set([200])
    for (index, kk) in enumerate(data):

        if kk['status'] != 200: 
            feilsituasjoner.append( kk )

        if index % 2 == 0 and index+1 < len(data):
            kv3 = data[index+1]
            statusvariant = set( [ kk['status'], kv3['status']])
            if statusvariant != normalstatus and 200 in statusvariant: 
                differanse.append( kk )
                differanse.append( kv3 )

    print('Antall kall som lykkes i den ene Api-versjonen men feiler i den andre', len( differanse ) )

#%% 
    retta = 0
    framleisfeil = 0 
    for (index, kk) in enumerate(feilsituasjoner):
        r = requests.get( kk['url'], headers={'Accept' : 'application/json'})
        if r.ok: 
            if len(r.text) > 100: 
                res = r.json()
            else: 
                res = {}
            if 'referanse' in res.keys():
                retta += 1
            else: 
                framleisfeil += 1 
        else: 
        ...
        if index == 10 or index % 50 == 0: 
             print( 'Sjekker feilsituasjon', index, 'av', len(feilsituasjoner))

    print( 'Antall feilsituasjoner vi prøvde om igjen med SUKSESS', retta, '/', len(feilsituasjoner), 
                                round( 100*retta/len(feilsituasjoner )), '%')
    print( 'Antall feilsituasjoner som fremdeles feiler', framleisfeil, '/', len(feilsituasjoner), 
                                round( 100*framleisfeil/len(feilsituasjoner )), '%')
